  ...
  return (
    <span className="group relative inline-flex">
      <button
        ...
        title="Password help"
        className="inline-flex h-5 w-5 items-center justify-center rounded-lg border border-[var(--line-soft)] bg-white text-[10px] font-semibold leading-none text-[var(--ink-muted)] transition hover:border-[var(--line-strong)] hover:text-[var(--ink-strong)]"
      >
        i
      </button>
      <span className="pointer-events-none absolute bottom-full left-1/2 z-30 mb-2 hidden w-max max-w-[260px] -translate-x-1/2 rounded-lg border border-[var(--line-soft)] bg-[rgb(15,23,42)] px-2.5 py-1.5 text-xs font-medium normal-case tracking-normal text-white shadow-[0_12px_28px_rgba(15,23,42,0.18)] group-hover:block group-focus-within:block">
...
export function UserEditorModal({
  ...
  const timezoneSelectOptions = useMemo(
    () => timezoneOptions.map((option) => ({ value: option, label: option })),
    [timezoneOptions],
  );
  const appRoleSelectOptions = useMemo(
    () => APP_ROLE_OPTIONS.map((option) => ({ value: option.value, label: option.label })),
    [],
  );
      ...
      >
        ...
        <div className="space-y-4 px-5 py-5">
              ...
              <div className="space-y-2">
                ...
                <div>
                  <SearchableSelect
                    ...
                    disabled={isSaving}
                    ariaLabel="Timezone"
                    inputClassName="h-9 border-[var(--line-strong)] bg-white px-3 pr-8 text-[13px]"
              ...
              <div className="space-y-2">
                ...
                <label className="flex h-9 items-center justify-between rounded-lg border border-[var(--line-strong)] bg-white px-3 text-[13px] text-[var(--ink-strong)]">
                  ...
                  <input
                    ...
                    className="h-4 w-4 rounded-lg accent-[var(--accent-strong)] disabled:cursor-not-allowed"
                  />
                </label>
              </div>
            </div>
          ...
          <div className="space-y-3 rounded-lg border border-[var(--line-soft)] bg-[var(--surface-card)] p-4">
            <div>
              <p className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--ink-subtle)]">
                New password
