...
type WorkspaceMemberRemoveTarget = {
  workspace: WorkspaceAdminItem;
  member: WorkspaceAdminMemberItem;
};

export function WorkspaceMemberRemoveModal({
  target,
  onClose,
  onConfirm,
  isSaving,
  ...
  target: WorkspaceMemberRemoveTarget | null;
  onClose: () => void;
      ...
      >
        <div className="border-b border-[var(--line-soft)] bg-white px-5 py-4">
          <div className="space-y-1.5">
            <ModalHeaderKicker code={`USR-${target.member.userId}`} tone="danger" />
            <h2
              id="remove-workspace-member-title"
