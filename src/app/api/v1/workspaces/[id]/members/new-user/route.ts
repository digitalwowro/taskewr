...
import type { WorkspaceMemberNewUserInput } from "@/domain/workspaces/schemas";
...
export async function POST(
  ...
  try {
    ...
    await assertMutationRateLimit(request, "workspaces:members:create-user");
    const { id } = await params;
    const body = await parseJsonBody<WorkspaceMemberNewUserInput>(request);
    const workspace = await service.createAndAddMember(
